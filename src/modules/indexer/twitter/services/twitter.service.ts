...
} from 'src/modules/database/qdrant/services/qdrant-bot-replies.repository';
...
export class TwitterService {
  ...
  async checkForBotMentionsAndRespondIfNeeded(): Promise<any> {
    try {
      ...
      const alreadyRespondedTweets =
        await this.qdrantBotRepliesRepository.findRepliesByInResponseTo(
          mentions.map((m) => m.id),
        );

      const notRespondedMentions = mentions.filter(
        (m) => !alreadyRespondedTweets.some((r) => r.in_respond_to === m.id),
      );

      for (const mention of notRespondedMentions) {
  ...
  async respondToTweet(
    ...
    if (isReply) {
      tweetsToProcess = await this.twitterApiService.getThreadByConversationId(
    ...
    const textToProcess =
      this.transformTweetsToSendToOrchestrator(tweetsToProcess);
    ...
    console.log(`Orchestrator response: "${orchestratorResponse.response}"`);

    let response_twit_id = null;

    if (!orchestratorResponse.messageNotRequireAnswer) {
      const responseTwitData = await this.twitterApiService.postComment(
