...
export abstract class BaseIndexerService {
  ...
  constructor(
    protected readonly unifiedStorage: UnifiedStorageService,
  ) {
  ...
  async runIndexer(): Promise<IndexingResult> {
    ...
    try {
      ...
      this.logger.error(`${config.serviceName} indexer failed: ${error.message}`);
   ...
   */
  protected async processBatches(documents: MasterDocument[]): Promise<BatchProcessingResult> {
    const config = this.getIndexerConfig();
    ...
    for (let i = 0; i < documents.length; i += batchSize) {
      ...
      try {
        this.logger.debug(`Processing batch ${batchNumber}/${totalBatches} (${batch.length} documents)`);
        ...
        this.logger.error(`Batch ${batchNumber} processing failed: ${error.message}`);
  ...
  async getHealth(): Promise<IndexerHealth> {
    ...
    const health: IndexerHealth = {
      ...
      collectionName: 'unified_messages',
    ...
    try {
      ...
      if (!storageHealth.isHealthy) {
        ...
        health.errors.push(`Storage dependency unhealthy: ${storageHealth.error}`);
      }
   ...
   */
  private async checkStorageHealth(): Promise<{ name: string; isHealthy: boolean; error?: string; responseTime?: number }> {
    const startTime = Date.now();
   ...
   */
  protected async checkExternalApiHealth(): Promise<{ name: string; isHealthy: boolean; error?: string; responseTime?: number } | undefined> {
