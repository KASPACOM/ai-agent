...
export class IndexerConfigService {
  ...
  getUnifiedMessagesCollectionName(): string {
    return this.configService.get<string>('INDEXER_UNIFIED_COLLECTION_NAME', 'unified_messages');
  }
  ...
  getDefaultProcessingDelayMs(): number {
    return this.configService.get<number>('INDEXER_DEFAULT_PROCESSING_DELAY_MS', 1000);
  }
  ...
  getTelegramMaxConsecutiveFailures(): number {
    return this.configService.get<number>('INDEXER_TELEGRAM_MAX_CONSECUTIVE_FAILURES', 5);
  }

  getTelegramScheduleCron(): string {
    return this.configService.get<string>('INDEXER_TELEGRAM_SCHEDULE_CRON', '0 0 20 * * *');
  }
  ...
  getTwitterMaxConsecutiveFailures(): number {
    return this.configService.get<number>('INDEXER_TWITTER_MAX_CONSECUTIVE_FAILURES', 5);
  }

  getTwitterScheduleCron(): string {
    return this.configService.get<string>('INDEXER_TWITTER_SCHEDULE_CRON', '0 */15 * * * *');
  }
