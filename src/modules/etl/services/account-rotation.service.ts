...
export class AccountRotationService {
  ...
  async updateAccountStatus(
    ...
    const updated: Partial<AccountStatus> = {
      ...
      syncedTweets: (existing?.syncedTweets || 0) + result.tweetsProcessed,
      // Only update lastPartialSync if we actually processed tweets
      ...(result.tweetsProcessed > 0 && { lastPartialSync: now }),
      updatedAt: now,
      consecutiveRuns: (this.processingSession.get(account) || 0) + 1,
    };

    // Update completion status
    if (result.wasCompleted && !result.hasMoreData) {
  ...
  private async getAllAccountStatuses(): Promise<AccountWithStatus[]> {
    const configuredAccounts = this.etlConfig.getTwitterAccounts();
  ...
  private allocateRequests(
    ...
    for (const account of scoredAccounts) {
      ...
      const allocatedRequests = Math.min(
        remainingRequests,
      ...
      if (allocatedRequests > 0) {
        ...
        remainingRequests -= allocatedRequests;
      }
    }
  ...
  private logSelectionRationale(
    ...
    selected.forEach((selection, index) => {
      this.logger.log(
        `  ${index + 1}. @${selection.account} (${selection.allocatedRequests} requests) - ${selection.reason}`,
      );
    });
  }
  ...
  private async upsertAccountStatus(
    ...
    try {
      ...
      const fullStatus: AccountStatus = {
        ...
        syncedTweets:
          (existingStatus?.syncedTweets || 0) + (updates.syncedTweets || 0), // Accumulate
      ...
      this.logger.debug(
        `✅ Updated status for @${normalizedAccount}: ${updates.syncedTweets || 0} tweets, ${updates.requestsUsed || 0} requests`,
      );
    } catch (error) {
      this.logger.error(
